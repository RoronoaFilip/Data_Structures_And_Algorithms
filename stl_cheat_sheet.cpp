...
struct CustomStruct {
  ...
  bool operator<(const CustomStruct &rhs) const { return this->i > rhs.i; }
};
...
int main() {
  ...
  auto st_hook =
      st.find(42); // Valid iterator to the element if found, otherwise st.end()
  ...
  auto ust_five =
      ust.find(5); // Iterator to the element if found, otherwise ust.end()
  ...
  auto umst_five =
      umst.find(5); // Iterator to the element if found, otherwise ust.end()
  ...
  auto mp_five =
      mp.find(5); // Iterator to the element if found, mp.end() otherwise
  ...
  auto mmp_five =
      mmp.find(5); // Iterator to the element if found, mp.end() otherwise
  ...
  auto ump_five =
      ump.find(5); // Iterator to the element if found, mp.end() otherwise
  ...
  sort(vec.begin(), vec.end(),
       [](auto a, auto b) { return a > b; }); // Sort with custom function
  next_permutation(vec.begin(), vec.end());   // Get the next permutation
  replace(vec.begin(), vec.end(), 0,
          42); // Replace all occurrences of 0 with 42
  auto pivot_it = partition(vec.begin(), vec.end(), [](auto a) {
    return a < 15;
  }); // Partition and return iterator to pivot
  nth_element(vec.begin(), vec.begin() + 2,
              vec.end()); // Rearrange elements so nth is the pivot
  int count_42s =
      count(vec.begin(), vec.end(), 42); // Count the occurrences of 42
  auto first_42 = find(vec.begin(), vec.end(),
                       42); // Iterator to the first occurrence of 42
  auto vec_lower = lower_bound(vec.begin(), vec.end(),
                               42); // Iterator to the first element >= 42
  auto vec_upper = upper_bound(vec.begin(), vec.end(),
                               42); // Iterator to the first element > 42
  bool found_42 = binary_search(
      vec.begin(), vec.end(), 42); // True if 42 exists in the sorted collection
  // Shuffle
